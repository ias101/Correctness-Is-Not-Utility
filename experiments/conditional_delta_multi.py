"""
Conditional Delta Probe — multi-regime version (Loop 27, external-validity fix).

Identical protocol to conditional_delta_v5.py (the canonical HotpotQA analysis),
parameterized by dataset so we can run a matched second-regime replication.

  python conditional_delta_multi.py --data <jsonl> --out <json> --tag <name>

Schema required (same as canonical V5):
  per record: query_id, stage_idx, stage_correctness, multi_layer_hidden_states (4 x 3584)

Reports (all query-grouped, 5-fold GroupKFold, 5000 bootstraps):
  cond_benefit | wrong_t, cond_degradation | correct_t, conditional asymmetry,
  correctness-leakage controls, joint replication, and joint-probe cross-eval
  within the conditional subsets.
"""
...
print(f"[{args.tag}] Loading {args.data} ...")
...
y_wrong = (cur_arr == 0).astype(int)

print(f"[{args.tag}] Transitions: {len(X)}, feat dim: {X.shape[1]}")
...
out = {"_meta": {"tag": args.tag, "data": args.data, "n_boot": N_BOOT,
                 "feat_dim": int(X.shape[1])}}
rng = np.random.RandomState(42)

wrong_mask = cur_arr == 0
correct_mask = cur_arr == 1
Xw, qw, yw = X[wrong_mask], qids[wrong_mask], nxt_arr[wrong_mask]
Xc, qc, yc = X[correct_mask], qids[correct_mask], (1 - nxt_arr)[correct_mask]
...
# conditional asymmetry (deg - ben, MLP)
...
for _ in range(N_BOOT):
    ...
    if len(iw) == 0 or len(ic) == 0 or len(set(yw[iw])) < 2 or len(set(yc[ic])) < 2:
        continue
...
out["cond_asymmetry_deg_minus_ben_mlp"] = {
    ...
    "p_deg_ge_ben": float((asym >= 0).mean()), "n_boot_valid": int(len(asym))}
...
out["joint_benefit_mlp_full"] = boot_ci(y_ben, mlp_jb, qids, rng)
out["joint_degradation_mlp_full"] = boot_ci(y_deg, mlp_jd, qids, rng)
out["joint_benefit_lr_full"] = boot_ci(y_ben, lr_jb, qids, rng)
out["joint_degradation_lr_full"] = boot_ci(y_deg, lr_jd, qids, rng)
...
# per-stage conditional breakdown
...
print("\n" + "=" * 70)
print(f"CONDITIONAL DELTA PROBE — {args.tag}")
print("=" * 70)
for k in ["cond_benefit_lr", "cond_benefit_mlp", "cond_degradation_lr", "cond_degradation_mlp",
          "leakage_wrongness_as_benefit_mlp", "correctness_direction_auroc_mlp",
