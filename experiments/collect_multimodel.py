"""
Multi-model hidden-state collection on HotpotQA.

Supports Qwen2.5-7B, Mistral-7B, and any HF causal LM.
Uses progressive context (2→4→6→8 cross-encoder-ranked passages)
with richer hidden-state extraction (4-layer hooks + mean pooling).

Usage:
  python collect_multimodel.py --model mistralai/Mistral-7B-Instruct-v0.3 \
      --num_queries 500 --output data/collected_hotpotqa_mistral.jsonl
  python collect_multimodel.py --model Qwen/Qwen2.5-7B-Instruct \
      --num_queries 500 --output data/collected_hotpotqa_qwen.jsonl
"""
...
def rank_bm25(query, paragraphs, top_k):
...
def load_hotpotqa(num_queries, split="validation"):
    ...
    for i, ex in enumerate(ds):
        ...
        queries.append({
            "id": str(i), "question": ex["question"],
            "answers": [ex["answer"]], "paragraphs": paragraphs,
...
def collect(queries, model, tokenizer, store, output_path, model_name="unknown"):
    collected = []
    n_final_correct = 0
    ...
    for q_idx, query in enumerate(tqdm(queries, desc=f"Collecting {short_name}")):
    ...
    elapsed = time.time() - t0
    n = len(queries)
    print(f"\n[*] {short_name}: {n} queries, {len(collected)} tuples in {elapsed:.0f}s")
...
def extract_hidden_state_multimodel(model, tokenizer, prompt, store, max_length=2048):
    ...
    if mean_key in store:
        result["mean_pool"] = store[mean_key][0, :]
    else:
    ...
    if "-1" not in store and last_layer in store:
        store["-1"] = store[last_layer]
...
def main():
    ...
    ap.add_argument("--output", type=str, default="data/collected_hotpotqa_multimodel.jsonl")
    ...
    print(f"[*] V4 mode: progressive context {STAGE_CONTEXT_SIZES}, 4-layer hooks + mean pool")

    queries = load_hotpotqa(args.num_queries)
    ...
    try:
        ...
        print(f"\n[*] Stats: {len(set(d['query_id'] for d in data))} queries, "
              f"{len(data)} tuples, label-positive rate {100*n_correct/len(data):.1f}%")
    finally:
