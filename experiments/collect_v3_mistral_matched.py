"""
MATCHED MISTRAL collector (Loop 31 / A1): collect_states_hotpot_v3.py with the dataset path
fixed to `hotpotqa/hotpot_qa` and split=validation, defaulting to Mistral-7B-Instruct-v0.3.
Run as-is to collect Mistral hidden states (16-bit full precision; pass --use_4bit for 4-bit)
on validation[0:2000] = the EXACT Qwen-V5 query set (proven byte-identical in
results/v5_experiment/mistral_v5_qid_manifest.json). The reported matched Mistral run used full
precision; the Qwen reference is 4-bit, but the 4-bit/8-bit control (cosine >0.99) makes the
precision difference immaterial. Pass --model_name to collect a different model on the same queries.
...
def load_hotpotqa(num_queries, split="validation"):
    ...
    ds = load_dataset("hotpotqa/hotpot_qa", "distractor", split=f"{split}[:{num_queries}]")
    queries = []
...
def main():
    ...
    ap.add_argument("--model_name", type=str, default="mistralai/Mistral-7B-Instruct-v0.3")  # matched-Mistral collector (Loop 31); LLAMA_MODEL_NAME (Qwen) kept importable for reference
    ...
    ap.add_argument("--use_4bit", action="store_true",
                    help="enable 4-bit quantization; DEFAULT is 16-bit full precision, as used in the reported matched Mistral run")
    args = ap.parse_args()
    ...
    model, tokenizer, hook_handles, store = load_llama_with_hidden_states(
        model_name=args.model_name, use_4bit=args.use_4bit,
    )
    try:
